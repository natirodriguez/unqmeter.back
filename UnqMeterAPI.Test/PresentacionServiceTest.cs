...
namespace UnqMeterAPI.Test
{
    ...
    public class PresentacionServiceTest
    {
        ...
        public void SinFechaSeteada_CompartirPresentacion_SeteaCorrectamenteFechaInicioFin()
        {
            ...
            var result = _presentacionService?.CompartirPresentacion(1) != null ? _presentacionService.CompartirPresentacion(1) : new Presentacion();

            Assert.IsNotNull(result?.FechaInicioPresentacion);
            Assert.IsNotNull(result?.FechaFinPresentacion);

            var startDate = result?.FechaInicioPresentacion != null ? result.FechaInicioPresentacion : DateTime.Now;
            var endDate = result?.FechaFinPresentacion != null ? result.FechaFinPresentacion : DateTime.Now;

            Assert.AreEqual(startDate.Value.AddDays(1), endDate.Value);
        }
